...
export async function POST(req: NextRequest) {
  try {
    ...
    const fp = visitor_fingerprint || visitor_id || body.fingerprint || `fp_${Math.random().toString(36).substring(2, 10)}`;
    ...
    const visitorRows = await query<{ id: string }>(
      ...
      RETURNING id`,
      [profile_username, fp, ip, country, city, region, device_type || 'desktop', browser || 'Unknown', os || 'Unknown', referrer || '']
    );
