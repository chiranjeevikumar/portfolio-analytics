...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalVisitorsRes = await query<{ count: string }>(
      `SELECT COUNT(*) as count FROM visitors WHERE profile_username = $1`,
      [username]
    );
    ...
    const uniqueVisitorsRes = await query<{ count: string }>(
      `SELECT COUNT(DISTINCT visitor_fingerprint) as count FROM visitors WHERE profile_username = $1`,
      [username]
    );
    ...
    const visitorsTodayRes = await query<{ count: string }>(
      `SELECT COUNT(DISTINCT visitor_id) as count FROM page_views WHERE profile_username = $1 AND created_at >= CURRENT_DATE`,
      [username]
    );
    ...
    const projectViewsTodayRes = await query<{ count: string }>(
      `SELECT COUNT(*) as count FROM page_views WHERE profile_username = $1 AND page_type IN ('project', 'demo_video', 'live_demo_click') AND created_at >= CURRENT_DATE`,
      [username]
    );
    ...
    const demoClicksTodayRes = await query<{ count: string }>(
      `SELECT COUNT(*) as count FROM page_views WHERE profile_username = $1 AND page_type IN ('demo_video', 'live_demo_click') AND created_at >= CURRENT_DATE`,
      [username]
    );
    ...
    const visitorsWeekRes = await query<{ count: string }>(
      `SELECT COUNT(DISTINCT visitor_id) as count FROM page_views WHERE profile_username = $1 AND created_at >= (NOW() - INTERVAL '7 days')`,
      [username]
    );
    ...
    const totalLeadsRes = await query<{ count: string }>(
      `SELECT COUNT(*) as count FROM connections WHERE profile_username = $1`,
      [username]
    );
    ...
    const totalProjectViewsRes = await query<{ count: string }>(
      `SELECT COUNT(*) as count FROM page_views WHERE profile_username = $1 AND page_type IN ('project', 'demo_video', 'live_demo_click')`,
      [username]
    );
